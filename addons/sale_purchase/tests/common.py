...
classTestCommonSalePurchaseNoChart(TestSaleCommon):
    ...
    defsetUpClass(cls,chart_template_ref=None):
        ...
        cls.product_category_purchase=cls.env['product.category'].create({
            'name':'ProductCategorywithIncomeaccount',
        ...
        cls.service_purchase_2=cls.env['product.product'].create({
            'name':"Out-sourcedService2",
            'standard_price':20.0,
            'list_price':15.0,
            'type':'service',
            'uom_id':uom_dozen.id, #differentUoM
            'uom_po_id':uom_unit.id,
