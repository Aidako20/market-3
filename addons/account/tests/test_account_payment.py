...
classTestAccountPayment(AccountTestInvoicingCommon):
    ...
    deftest_payment_move_sync_create_write(self):
        ...
        #====Checkeditingtheaccount.payment====
        ...
        #====Checkeditingtheaccount.move.line====
    ...
    deftest_payment_move_sync_onchange(self):
        copy_receivable=self.copy_account(self.company_data['default_account_receivable'])
        ...
        withmove_form.line_ids.edit(1)asline_form:
            line_form.currency_id=self.company_data['currency']
    ...
    deftest_inbound_payment_sync_writeoff_debit_sign(self):
        payment=self.env['account.payment'].create({
            'amount':100.0,
            'payment_type':'inbound',
            'partner_type':'customer',
    ...
    deftest_inbound_payment_sync_writeoff_credit_sign(self):
        payment=self.env['account.payment'].create({
            'amount':100.0,
            'payment_type':'inbound',
            'partner_type':'customer',
    ...
    deftest_outbound_payment_sync_writeoff_debit_sign(self):
        payment=self.env['account.payment'].create({
            'amount':100.0,
    ...
    deftest_outbound_payment_sync_writeoff_credit_sign(self):
        payment=self.env['account.payment'].create({
            'amount':100.0,
            'payment_type':'outbound',
            'partner_type':'supplier',
    ...
    deftest_internal_transfer(self):
        copy_receivable=self.copy_account(self.company_data['default_account_receivable'])
    ...
    deftest_compute_currency_id(self):
        '''Whencreatinganewaccount.paymentwithoutspecifyingacurrency,thedefaultcurrencyshouldbetheone
        setonthejournal.
        '''
    ...
    deftest_reconciliation_payment_states(self):
        payment=self.env['account.payment'].create({
            'amount':50.0,
            'payment_type':'inbound',
            'partner_type':'customer',
        ...
        (counterpart_lines+invoice.line_ids.filtered(lambdaline:line.account_internal_type=='receivable'))\
            .reconcile()
    ...
    deftest_payment_name(self):
        ...
        withForm(AccountPayment.with_context(default_move_journal_types=('bank','cash')))aspayment_form:
    ...
    deftest_payment_form_onchange_journal(self):
    ...
    deftest_suggested_default_partner_bank(self):
        """Ensurethe'partner_bank_id'iswellcomputedonpayments.Whenthepaymentisinbound,themoneymustbe
        receivedbyabankaccountlinkedtothecompany.Incaseofoutboundpayment,thebankaccountmustbefound
        onthepartner.
        """
        payment=self.env['account.payment'].create({
        ...
        #Asequenceisautomaticallyaddedonthefirstmove.Weneedtocleanitbeforechangingthejournal.
        payment.name=False
    ...
    deftest_internal_transfer_custom_partner_bank_id(self):
        """Ensurepartner_bank_iduserchoiceisnotsystematicallyignoredbycomputemethod."""
    ...
    deftest_internal_transfer_change_journal(self):
        self.bank_journal_1.bank_account_id=self.comp_bank_account1
