...
fromflectra.addons.microsoft_calendar.tests.commonimportTestCommon,mock_get_token
fromflectra.exceptionsimportValidationError

@patch.object(User,'_get_microsoft_calendar_token',mock_get_token)
classTestCreateEvents(TestCommon):

    deftest_create_simple_event_without_email(self):
        """
        Outlookdoesnotacceptattendeeswithoutemail.
        """
        #arrange
    ...
    deftest_create_simple_event_from_outlook_organizer_calendar(self,mock_get_events):
        """
        AneventhasbeencreatedinOutlookandsyncedintheFlectraorganizercalendar.
        """
    ...
    @patch.object(MicrosoftCalendarService,'get_events')
    deftest_create_simple_event_from_outlook_attendee_calendar_and_organizer_exists_in_flectra(self,mock_get_events):
        """
        AneventhasbeencreatedinOutlookandsyncedintheFlectraattendeecalendar.
        ThereisaFlectrauserthatmatcheswiththeorganizeremailaddress.
        """
    ...
    @patch.object(MicrosoftCalendarService,'get_events')
    deftest_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_flectra(
        self,mock_get_events
    ):
        """
        AneventhasbeencreatedinOutlookandsyncedintheFlectraattendeecalendar.
        noFlectrauserthatmatcheswiththeorganizeremailaddress.
        """
        ...
        outlook_event=dict(self.simple_event_from_outlook_attendee,organizer={
            'emailAddress':{'address':"john.doe@flectrahq.com",'name':"JohnDoe"},
        })
    ...
    deftest_create_recurrent_event_with_sync(self,mock_insert,mock_get_events):
        """
        AFlectrarecurrenteventiscreatedwhenOutlooksyncisenabled.
        """

        #>>>firstphase:createtherecurrence
        ...
        self.assertEqual(recurrence.name,"Every2Days,for7events")
        self.assertEqual(len(recurrence.calendar_event_ids),7)

        #>>>secondphase:syncwithorganizeroutlookcalendar
    ...
    @patch.object(MicrosoftCalendarService,'get_events')
    @patch.object(MicrosoftCalendarService,'insert')
    deftest_create_recurrent_event_with_sync_by_another_user(self,mock_insert,mock_get_events):
        """
        AFlectrarecurrenteventhasbeencreatedandsyncedwithOutlookbyanotheruser,butnothing
        shouldhappenasitwepreventsyncofrecurrencesfromotherusers
        (seemicrosoft_calendar/models/calendar_recurrence_rule.py::_get_microsoft_sync_domain())
        """
        ...
        mock_insert.assert_not_called()
        self.assertEqual(record.name,"recurring_event")
        self.assertEqual(recurrence.name,f"Every2Days,for{self.recurrent_events_count}events")
        self.assertEqual(len(recurrence.calendar_event_ids),self.recurrent_events_count)

        #>>>secondphase:syncwithattendeeOutlookcalendar
    ...
    @patch.object(MicrosoftCalendarService,'get_events')
    deftest_create_recurrent_event_from_outlook_organizer_calendar(self,mock_get_events):
        """
        ArecurrenteventhasbeencreatedinOutlookandsyncedintheFlectraorganizercalendar.
        """
    ...
    @patch.object(MicrosoftCalendarService,'get_events')
    deftest_create_recurrent_event_from_outlook_attendee_calendar(self,mock_get_events):
        """
        ArecurrenteventhasbeencreatedinOutlookandsyncedintheFlectraattendeecalendar.
        """
