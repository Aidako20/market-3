...
classTestSmsTemplate(TestMailFullCommon,TestRecipients):
    ...
    defsetUpClass(cls):
        ...
        cls.body_en='Dear${object.display_name}thisisanSMS.'
        cls.body_fr=u"Hello${object.display_name}ceciestenfrançais."
        cls.sms_template=cls._create_sms_template('mail.test.sms',body=cls.body_en)

    deftest_sms_template_render(self):
        rendered_body=self.sms_template._render_template(self.sms_template.body,self.sms_template.model,self.test_record.ids)
        self.assertEqual(rendered_body[self.test_record.id],'Dear%sthisisanSMS.'%self.test_record.display_name)

    deftest_sms_template_lang(self):
        ...
        forlang,(tpl,rids)intpl_to_rids.items():
            ...
            eliflang=='fr_FR':
                ...
                self.assertTrue(False,'Shouldnotreturnlang%s'%lang)

    deftest_sms_template_create_and_unlink_sidebar_action(self):
    ...
    deftest_sms_template_unlink_with_action(self):
        ActWindow=self.env['ir.actions.act_window']
        self.sms_template.action_create_sidebar_action()
        action_id=self.sms_template.sidebar_action_id.id
