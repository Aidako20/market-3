...
classTestPhoneBlacklist(TestMailFullCommon,TestRecipients):
    """TODO"""
    ...
    deftest_phone_blacklist_internals(self):
    ...
    deftest_phone_sanitize_api(self):
        withself.with_user('employee'):
            test_record=self.env['mail.test.sms.bl'].browse(self.test_record.id)
            self.assertFalse(test_record.phone_sanitized_blacklisted)
    ...
    deftest_phone_sanitize_internals(self):
        withself.with_user('employee'):
            test_record=self.env['mail.test.sms.bl'].browse(self.test_record.id)
