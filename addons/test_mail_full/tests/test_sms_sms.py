...
classLinkTrackerMock(common.BaseCase):
    ...
    defassertLinkTracker(self,url,url_params):
...
classTestSMSPost(TestMailFullCommon,LinkTrackerMock):
    ...
    deftest_body_link_shorten(self):
        link='http://www.example.com'
        self.env['link.tracker'].search([('url','=',link)]).unlink()
        new_body=self.env['mail.render.mixin']._shorten_links_text('Welcometo%s!'%link,self.tracker_values)
    ...
    deftest_body_link_shorten_wunsubscribe(self):
    ...
    deftest_sms_body_link_shorten_suffix(self):
        mailing=self.env['mailing.mailing'].create({
            'subject':'Minimalmailing',
            'mailing_model_id':self.env['ir.model']._get('mail.test.sms').id,
            'mailing_type':'sms',
        })

        sms_0=self.env['sms.sms'].create({
            'body':'Welcometohttps://test.flectrahq.com',
            'number':'12',
            'mailing_id':mailing.id,
        })
        sms_1=self.env['sms.sms'].create({
            'body':'Welcometohttps://test.flectrahq.com/r/RAOUL',
            'number':'12',
        })
        ...
        sms_3=self.env['sms.sms'].create({
            'body':'Welcometohttps://test.flectrahq.com/leodagan/r/RAOUL',
            'number':'12','mailing_id':mailing.id,
        })

        res=(sms_0|sms_1|sms_2|sms_3)._update_body_short_links()
        self.assertEqual(res[sms_0.id],'Welcometohttps://test.flectrahq.com')
        self.assertEqual(res[sms_1.id],'Welcometohttps://test.flectrahq.com/r/RAOUL')
        self.assertEqual(res[sms_2.id],'Welcometohttps://test.flectrahq.com/r/RAOUL/s/%s'%sms_2.id)
        self.assertEqual(res[sms_3.id],'Welcometohttps://test.flectrahq.com/leodagan/r/RAOUL')

    deftest_sms_send_batch_size(self):
        self.count=0

        def_send(sms_self,delete_all=False,raise_exception=False):
            self.count+=1
            returnDEFAULT

        self.env['ir.config_parameter'].set_param('sms.session.batch.size','3')
        withpatch.object(SmsSms,'_send',autospec=True,side_effect=_send)assend_mock:
            self.env['sms.sms'].browse(self.sms_all.ids).send()

        self.assertEqual(self.count,4)

    deftest_sms_send_crash_employee(self):
        withself.assertRaises(exceptions.AccessError):
            self.env['sms.sms'].with_user(self.user_employee).browse(self.sms_all.ids).send()

    deftest_sms_send_delete_all(self):
        withself.mockSMSGateway(sms_allow_unlink=True,sim_error='jsonrpc_exception'):
    ...
    deftest_sms_send_raise(self):
        withself.assertRaises(exceptions.AccessError):
            withself.mockSMSGateway(sim_error='jsonrpc_exception'):
    ...
    deftest_sms_send_raise_catch(self):
        withself.mockSMSGateway(sim_error='jsonrpc_exception'):
