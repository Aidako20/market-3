...
fromflectra.addons.website.tests.test_website_visitorimportMockVisitor
fromflectra.addons.website_event.tests.commonimportEventDtPatcher


classTestEventFullCommon(TestEventCrmCommon,TestSalesCommon,EventDtPatcher,MockVisitor):
    ...
    defsetUpClass(cls):
        ...
        cls.event_0.write({
            #eventif8-18inEurope/Brussels(DST)(firstday:beginsat9,lastday:endsat15)
    ...
    defassertLeadConvertion(self,rule,registrations,partner=None,**expected):
        ...
        forregistrationinregistrations:
            ifnotregistration.registration_answer_ids:
                continue
            foranswerinregistration.registration_answer_ids:
                self.assertIn(answer.question_id.title,lead.description)
                ifanswer.question_type=='simple_choice':
                    self.assertIn(answer.value_answer_id.name,lead.description)
                else:
                    self.assertIn(answer.value_text_box,lead.description) #better:checkmultiline


classTestWEventCommon(HttpCaseWithUserDemo,HttpCaseWithUserPortal,EventDtPatcher,MockVisitor):

    defsetUp(self):
        super(TestWEventCommon,self).setUp()

        self.event_product=self.env['product.product'].create({
            'name':'TestEventRegistration',
        ...
        self.env['event.event'].search(
            [('name','like','%OnlineReveal%')]
        ).write(
            {'name':'Donotclickonme'}
        )
        self.event=self.env['event.event'].create({
            'name':'OnlineRevealTestEvent',
            'auto_confirm':True,
            'stage_id':self.env.ref('event.event_stage_booked').id,
            'address_id':False,
            'user_id':self.user_demo.id,
            'tag_ids':[(4,self.event_tag_category_1_tag_1.id)],
            #eventif8-18inEurope/Brussels(DST)(firstday:beginsat7,lastday:endsat17)
        ...
        self.event_question_1=self.env['event.question'].create({
            'title':'Whichfieldareyouworkingin',
            'question_type':'simple_choice',
        ...
        self.env['event.meeting.room'].create({
            'name':'Bestwoodforfurniture',
            'summary':'Let\'stalkaboutwoodtypesforfurniture',
            'target_audience':'woodexpert(s)',
            'is_pinned':True,
