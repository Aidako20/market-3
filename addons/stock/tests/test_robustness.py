...
classTestRobustness(SavepointCase):
    @classmethod
    defsetUpClass(cls):
    ...
    deftest_uom_factor(self):
        """Changingthefactorofaunitofmeasureshouldn'tbeallowedwhile
        quantitiesarereserved,elsetheexistingmovelineswon'tbeconsistent
        withthe`reserved_quantity`onquants.
        """
        #makesomestock
    ...
    deftest_location_usage(self):
        """Changingtheusageofalocationshouldn'tbeallowedwhile
        quantitiesarereserved,elsetheexistingmovelineswon'tbe
        consistentwiththe`reserved_quantity`onthequants.
        """
        #changestockusage
        ...
        #assertthereservation
        self.assertEqual(quant.reserved_quantity,0) #reservationisbypassedinscraplocation
    ...
    deftest_package_unpack(self):
        """Unpackapackagethatcontainsquantswithareservation
        shouldalsoremovethepackageonthereservedmovelines.
        """
        package=self.env['stock.quant.package'].create({
            'name':'ShellHelixHX710W30',
        })
    ...
    deftest_lot_id_product_id_mix(self):
        """Makesureitisn'tpossibletocreateamovelinewithalotincompatiblewithits
        product.
        """
    ...
    deftest_unreserve_error(self):
        ...
        self.env.cr.execute("UPDATEstock_quantsetreserved_quantity=0WHEREid=%s",(quant.id,))
    ...
    deftest_unreserve_fix(self):
        ...
        quant=self.env['stock.quant']._gather(
            self.product1,self.stock_location,strict=True)
        self.env.cr.execute(
            "UPDATEstock_quantsetreserved_quantity=0WHEREid=%s",(quant.id,))
        quant.invalidate_cache()
        server_action=self.env.ref(
            'stock.stock_quant_stock_move_line_desynchronization',raise_if_not_found=False)
        ifnotserver_action:
    ...
    deftest_lot_quantity_remains_unchanged_after_done(self):
        """Makesurethemethod_set_lot_idsdoesnotchangethequantitiesoflotsto1oncetheyaredone.
        """
