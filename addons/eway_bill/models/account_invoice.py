...
class AccountInvoiceLine(models.Model):
    ...
    def action_invoice_open(self):
        ...
        auto_confirm_do = config_id.global_auto_confirm_delivery_order
        for invoice in self.filtered(
                lambda l: l.type in ['out_invoice', 'in_refund']):
            invoice.invoice_line_ids._action_launch_procurement_rule()
            if auto_confirm_do:
                invoice.mapped('picking_ids').action_assign()
                for picking_id in invoice.picking_ids:
                    action = picking_id.button_validate()
                    wizard = self.env[(action.get('res_model'))].browse(
                        action.get('res_id'))
                    wizard.process()
