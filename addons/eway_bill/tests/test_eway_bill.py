...
class TestCreateEway(TransactionCase):
    ...
    def test_invoice(self):
        ...
        self.account_recc_id = self.account_model.sudo(
            self.account_manager.id).create(dict(
                code="cust_acc",
        ...
        self.assertTrue(self.account_invoice_customer.picking_ids,
                        'Stock: no picking created for "invoice on delivery" '
                        'stockable products')
        ...
        self.assertTrue(pick.button_validate(),
                        'Stock: complete delivery should not need a backorder')
        del_qties = [inv.qty_delivered for inv in
                     self.account_invoice_customer.invoice_line_ids]
        del_qties_truth = [
            10.0 if inv.product_id.type in ['product', 'consu'] else 0.0 for
            inv in self.account_invoice_customer.invoice_line_ids]
        self.assertEqual(del_qties, del_qties_truth,
                         'Stock: delivered quantities are wrong after partial'
                         'deliver quantities are wrong after partial delivery')

        context_dict = {'active_model': 'account.invoice',
                        'active_ids': [self.account_invoice_customer.id]}
        eway_vals = self.eway_wizard.with_context(context_dict).default_get([])
        eway_vals.update({
            'sub_type_id': self.sub_type.id,
        })

        self.eway = self.eway_wizard.with_context(context_dict).create(
            eway_vals)
        self.eway.generate_eway()
        self.assertTrue(self.eway.id,
                        'Eway Bill: no eway bill created for invoice')
