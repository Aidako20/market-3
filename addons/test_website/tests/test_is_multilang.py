...
classTestIsMultiLang(flectra.tests.HttpCase):

    deftest_01_is_multilang_url(self):
        ...
        fordatain[None,{'post':True}]:#GET/POST
    ...
    deftest_02_url_lang_code_underscore(self):
        website=self.env['website'].browse(1)
        it=self.env.ref('base.lang_it').sudo()
        en=self.env.ref('base.lang_en').sudo()
        be=self.env.ref('base.lang_fr_BE').sudo()
        country1=self.env['res.country'].create({'name':"MySuperCountry"})

        it.active=True
        be.active=True
        website.domain='http://127.0.0.1:7073' #for_is_canonical_url
        website.default_lang_id=en
        ...
        #Note:thistestisindirectlytestingthe`ref=canonical`tagiscorrectlyset,
        #      asitisrequiredinorderfor`rel=alternate`tagstobeinsertedintheDOM
