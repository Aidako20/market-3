...
fromflectra.tests.commonimportHttpCase,new_test_user,tagged


@tagged("post_install","-at_install")
classTestCalendarController(HttpCase):
    defsetUp(self):
        super().setUp()
    ...
    deftest_accept_meeting_unauthenticated(self):
        ...
        self.assertEqual(res.status_code,200,"Responseshould=OK")
        self.event.attendee_ids[1].invalidate_cache()
        self.assertEqual(self.event.attendee_ids[1].state,"accepted","Attendeeshouldhaveaccepted")

    deftest_accept_meeting_authenticated(self):
        self.event.write({"partner_ids":[(4,self.other_user.partner_id.id)]})
        token=self.event.attendee_ids[1].access_token
        url="/calendar/meeting/accept?token=%s&id=%d"%(token,self.event.id)
