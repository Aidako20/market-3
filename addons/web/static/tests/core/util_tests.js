...
QUnit.module('core',{},function(){
    ...
    QUnit.test('groupBy',function(assert){
        ...
        assert.throws(
            ()=>groupBy({}),
            newTypeError(`listisnotiterable`)
        );
        assert.throws(
            ()=>groupBy([],true),
            newError(`Expectedcriterionoftype'string'or'function'andgot'boolean'`)
        );
        assert.throws(
            ()=>groupBy([],3),
            newError(`Expectedcriterionoftype'string'or'function'andgot'number'`)
        );
        assert.throws(
            ()=>groupBy([],{}),
            newError(`Expectedcriterionoftype'string'or'function'andgot'object'`)
        );
    ...
    QUnit.test('intersperse',function(assert){
        ...
        varintersperse=utils.intersperse;
    ...
    QUnit.test('is_bin_size',function(assert){
        ...
        varis_bin_size=utils.is_bin_size;

        assert.strictEqual(is_bin_size('Cg=='),false);
        assert.strictEqual(is_bin_size('2.5Mb'),true);
        //shouldalsoworkfornon-latinlanguages(e.g.russian)
        assert.strictEqual(is_bin_size('64.2Кб'),true);
    });

    QUnit.test('unaccent',function(assert){
        assert.expect(3);

        varsingleCharacters=utils.unaccent("ⱮɀꝾƶⱵȥ");
        vardoubledCharacters=utils.unaccent("ǱǄꝎꜩꝡƕ");
        varcaseSensetiveCharacters=utils.unaccent("ⱮɀꝾƶⱵȥ",true);
    ...
    QUnit.test('human_number',function(assert){
        ...
        varhuman_number=utils.human_number;
    ...
    QUnit.test('patchaclass',function(assert){
        assert.expect(4);
        ...
        constremovePatch=utils.patch(Child,'patch',{
            foo(){
                returnthis._super()+'patchfoo';
            },
            bar(){
                returnthis._super()+'patchbar';
            }
        })

        constchild=newChild();

        assert.strictEqual(child.foo(),'Parentfoopatchfoo')
        assert.strictEqual(child.bar(),'Childbarpatchbar')

        removePatch();

        assert.strictEqual(child.foo(),'Parentfoo');
        assert.strictEqual(child.bar(),'Childbar');
    })
    ...
    QUnit.test('sortBy',function(assert){
        assert.expect(27);
        const{sortBy}=utils;
        constbools=[true,false,true];
        constints=[2,1,5];
        conststrs=['b','a','z'];
        constobjbools=[{x:true},{x:false},{x:true}];
        constobjints=[{x:2},{x:1},{x:5}];
        constobjstrss=[{x:'b'},{x:'a'},{x:'z'}];

        //Invalid
        assert.throws(
            ()=>sortBy({}),
            newTypeError(`array.sliceisnotafunction`)
        );
        assert.throws(
            ()=>sortBy([Symbol('b'),Symbol('a')]),
            newTypeError(`CannotconvertaSymbolvaluetoanumber`)
        );
        assert.throws(
            ()=>sortBy(ints,true),
            newError(`Expectedcriterionoftype'string'or'function'andgot'boolean'`)
