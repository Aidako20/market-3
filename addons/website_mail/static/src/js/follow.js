...
publicWidget.registry.follow=publicWidget.Widget.extend({
    ...
    start:function(){
        ...
        varalways=function(data){
            ...
            const$jsFollowToEnable=$jsFollowEls.filter(function(){
                constmodel=this.dataset.object;
                returnmodelindata[1]&&data[1][model].includes(parseInt(this.dataset.id));
            });
        ...
        constrecords={};
        for(constelof$jsFollowEls){
            constmodel=el.dataset.object;
            if(!(modelinrecords)){
                records[model]=[];
            }
        ...
        //notifeditablemodetoallowdesignertoedit
    ...
    /**
     *Togglessubscriptionstateforeverygivenrecords.
     *
     *@private
    ...
    _toggleSubscription:function(follow,email,$jsFollowEls){
        if(follow){
            ...
            for(constelof$jsFollowEls){
                constfollow=!email&&el.getAttribute('data-unsubscribe');
                this._updateSubscriptionDOM(follow,email,$(el));
            }
        }
    },
    /**
     *UpdatessubscriptionDOMforeverygivenrecords.
     *Thisshouldnotbecalleddirectly,use`_toggleSubscription`.
     *
     *@private
     *@param{boolean}follow
     *@param{string}email
    ...
    _onClick:function(ev){
        ...
        varemail=$email.length?$email.val():false;
