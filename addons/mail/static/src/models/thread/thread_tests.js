...
QUnit.test('inbox&starredmailboxes',asyncfunction(assert){
    assert.expect(10);

    awaitthis.start();
    constmailboxInbox=this.env.messaging.inbox;
    constmailboxStarred=this.env.messaging.starred;
    assert.ok(mailboxInbox,"shouldhavemailboxinbox");
    assert.ok(mailboxStarred,"shouldhavemailboxstarred");
...
QUnit.test('create(channel)',asyncfunction(assert){
    assert.expect(23);
    ...
    constpartner9=this.env.models['mail.partner'].findFromIdentifyingData({id:9});
    constpartner10=this.env.models['mail.partner'].findFromIdentifyingData({id:10});
...
QUnit.test('create(chat)',asyncfunction(assert){
    assert.expect(15);
