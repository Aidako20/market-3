...
BasicModel.include({
     ...
     *@return{Promise<Object>}thepromiseisresolvedwiththe
     *                           invalidPartnerIds
     */
    _setInvalidMany2ManyTagsEmail:function(record,fieldName){
        ...
        varlist=this._applyX2ManyOperations(this.localData[localID]);
        varinvalidPartnerIds=[];
        ...
        if(invalidPartnerIds.length){
            //removeinvalidpartners
            varchanges={operation:'DELETE',ids:_.pluck(invalidPartnerIds,'id')};
...
varFieldMany2ManyTagsEmail=M2MTags.extend({
    ...
    /**
     *Openapopupforeachinvalidpartners(withoutemail)tofilltheemail.
     *
     *@private
     *@returns{Promise}
     */
    _checkEmailPopup:function(){
        ...
        returnPromise.all(popupDefs).then(function(){
            //Allpopupshavebeenprocessedforthegivenids
            //Itisnowtimetosetthefinalvaluewithvalidpartnersids.
            validPartners=_.uniq(validPartners);
            if(validPartners.length){
                varvalues=_.map(validPartners,function(id){
                    return{id:id};
                });
                self._setValue({
                    operation:'ADD_M2M',
    ...
    /**
     *Overridetocheckifallmany2manyvalueshaveanemailsetbefore
     *renderingthewidget.
     *
     *@override
     *@private
    ...
    _render:function(){
        ...
        returnnewPromise(function(resolve,reject){
            ...
            return_super.apply(self,arguments);
        });
    },
});
