...
fromflectra.addons.account_edi_extended.tests.commonimportAccountEdiExtendedTestCommon,_mocked_post,_mocked_post_two_steps,_generate_mocked_needs_web_services,_mocked_cancel_failed,_generate_mocked_support_batching
fromflectra.testsimporttagged


@tagged('post_install','-at_install')
classTestAccountEdi(AccountEdiExtendedTestCommon):
    ...
    deftest_edi_flow_two_steps(self):
        withself.mock_edi(_post_invoice_edi_method=_mocked_post_two_steps,
    ...
    deftest_edi_flow_request_cancel_success(self):
        withself.mock_edi(_needs_web_services_method=_generate_mocked_needs_web_services(True)):
    ...
    deftest_edi_flow_request_cancel_failed(self):
        withself.mock_edi(_needs_web_services_method=_generate_mocked_needs_web_services(True),
    ...
    deftest_edi_flow_two_step_cancel_with_call_off_request(self):
        def_mock_cancel(edi_format,invoices,test_mode):
        ...
        def_is_needed_for_invoice(edi_format,invoice):
            returnnotbool(invoice.ref)
        ...
        withself.mock_edi(_needs_web_services_method=_generate_mocked_needs_web_services(True),
            ...
            #RequestCancellation
            self.invoice.button_cancel_posted_moves()
            doc._process_documents_web_services(with_commit=False) #firststepofcancel
            self.assertEqual(doc.state,'to_cancel')
            ...
            #Ifwecannotcalloffedicancellation,onlysolutionistopostagain
            doc._process_documents_web_services(with_commit=False) #secondstepofcancel
            self.assertEqual(doc.state,'cancelled')
            self.invoice.action_post()
            doc._process_documents_web_services(with_commit=False)
            self.assertEqual(doc.state,'sent')
    ...
    deftest_batches(self):
        def_get_batch_key_method(edi_format,move,state):
            return(move.ref)

        withself.mock_edi(_get_batch_key_method=_get_batch_key_method,
