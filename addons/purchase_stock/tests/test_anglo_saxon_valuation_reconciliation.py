...
fromflectra.tests.commonimportForm,tagged
...
classTestValuationReconciliation(ValuationReconciliationTestCommon):
    ...
    def_create_purchase(self,product,date,quantity=1.0,set_tax=False,price_unit=66.0):
        rslt=self.env['purchase.order'].create({
            ...
            'order_line':[
                (0,0,{
                    ...
                    'date_planned':date,
                    'taxes_id':[(6,0,product.supplier_taxes_id.ids)]ifset_taxelseFalse,
                })],
             'date_order':date,
        })
        rslt.button_confirm()
    ...
    def_create_invoice_for_po(self,purchase_order,date):
    ...
    deftest_shipment_invoice(self):
        """Teststhecaseintowhichwereceivethegoodsfirst,andthenmaketheinvoice.
        """
    ...
    deftest_invoice_shipment(self):
        """Teststhecaseintowhichwemaketheinvoicefirst,andthenreceivethegoods.
        """
        #CreateaPOandaninvoiceforit
        ...
        #Validatetheinvoiceandrefundthegoods
        invoice.action_post()
    ...
    deftest_multiple_shipments_invoices(self):
        """Teststhecaseintowhichwereceivepartofthegoodsfirst,then2invoicesatdifferentrates,andfinallytheremainingquantities
        """
        test_product=self.test_product_delivery
        ...
        picking=self.env['stock.picking'].search([('purchase_id','=',purchase_order.id)],order="idasc",limit=1)
        ...
        #Wedon'tneedtomakethedateofprocessingexplicitsincetheverylastrate
        #willbetaken
        self._process_pickings(purchase_order.picking_ids.filtered(lambdax:x.state!='done'),quantity=3.0)
        picking=self.env['stock.picking'].search([('purchase_id','=',purchase_order.id)],order='iddesc',limit=1)
        self.check_reconciliation(invoice2,picking)

    deftest_rounding_discount(self):
        ...
        #Checkthepricedifferenceamount.
        price_diff_line=invoice.line_ids.filtered(lambdal:l.account_id==self.stock_account_product_categ.property_account_creditor_price_difference_categ)
        self.assertTrue(len(price_diff_line)==1,"Apricedifferencelineshouldbecreated")
    ...
    deftest_rounding_price_unit(self):
        ...
        #Checkthepricedifferenceamount.It'sexpectedthatprice_unit*qty!=price_total.
        price_diff_line=invoice.line_ids.filtered(lambdal:l.account_id==self.stock_account_product_categ.property_account_creditor_price_difference_categ)
        self.assertTrue(len(price_diff_line)==1,"Apricedifferencelineshouldbecreated")
    ...
    deftest_rounding_price_unit_exchange_difference(self):
        test_product=self.test_product_delivery
        ...
        price_diff_line=invoice.line_ids.filtered(lambdal:l.account_id==self.stock_account_product_categ.property_account_creditor_price_difference_categ)
        self.assertFalse(price_diff_line,"Nopricedifferencelineshouldbecreated")

        picking=self.env['stock.picking'].search([('purchase_id','=',purchase_order.id)])
        self.check_reconciliation(invoice,picking)

    @freeze_time('2021-01-03')
    deftest_price_difference_exchange_difference_accounting_date(self):
        test_product=self.test_product_delivery
        ...
        valuation_line=picking.move_lines.mapped('account_move_ids.line_ids').filtered(lambdax:x.account_id.id==interim_account_id)
        ...
        self.assertTrue(exchange_move,"Anexchangemoveshouldexists.")
        exchange_difference=exchange_move.line_ids.filtered(lambdal:l.account_id.id==interim_account_id).balance
        self.assertAlmostEqual(exchange_difference,38.46,msg="Exchangeamountisincorrect")

    deftest_reconcile_cash_basis_bill(self):
        '''TestthegenerationoftheCABAmoveafterbillpayment
        '''
