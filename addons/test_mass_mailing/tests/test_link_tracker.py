...
classTestLinkTracker(common.TestMassMailCommon):
    ...
    deftest_add_link(self):
        code=self.link.code
        self.assertEqual(self.link.count,1)

        #clickfromanewIPshouldcreateanewentry
        ...
        #clickfromsameIP(evenanothercountry)doesnotcreateanewentry
        click=self.env['link.tracker.click'].sudo().add_click(
            code,
            ip='100.00.00.01',
            country_code='FRA'
    ...
    deftest_add_link_mail_stat(self):
        mailing=self.env['mailing.mailing'].create({'name':'TestMailing',"subject":"Hi!"})
        code=self.link.code
        self.assertEqual(self.link.count,1)
        ...
        #clickfromanewIPshouldcreateanewentryandupdatestatwhenprovided
        click=self.env['link.tracker.click'].sudo().add_click(
            code,
            ip='100.00.00.01',
            country_code='BEL',
