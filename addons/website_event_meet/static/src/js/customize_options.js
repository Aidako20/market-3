...
letEventSpecificOptions=require('website_event.set_customize_options').EventSpecificOptions;
...
EventSpecificOptions.include({
    ...
    _onAllowRoomCreationChange:function(){
        letcheckboxValue=this.$allowRoomCreationInput.is(':checked');
    ...
    _getCheckboxFieldMatch:function(checkboxField){
        if(checkboxField==='meeting_room_allow_creation'){
            returnthis.$allowRoomCreationInput;
        }
        returnthis._super(checkboxField);
    },

    _initCheckboxCallback:function(rpcData){
        this._super(rpcData);
        if(rpcData[0]['meeting_room_allow_creation']){
            letsubmenuInput=this._getCheckboxFieldMatch('meeting_room_allow_creation');
            submenuInput.attr('checked','checked');
        }
    },

    _toggleAllowRoomCreation:asyncfunction(val){
        awaitthis._rpc({
