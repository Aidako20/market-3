...
classEventDtPatcher(SavepointCase):
...
classTestWebsiteEventCommon(TestEventCommon):
    ...
    defsetUpClass(cls):
        ...
        cls.user_event_web_manager=mail_new_test_user(
            ...
            name='MartinSalesManager',email='crm_manager@test.example.com',
    ...
    def_get_menus(self):
        returnset(['Introduction','Location','Register'])

    def_assert_website_menus(self,event,menu_entries=None):
        self.assertTrue(event.menu_id)

        ifmenu_entriesisNone:
        ...
        forpage_specificin['Introduction','Location']:
            view=self.env['ir.ui.view'].search(
                [('name','=',page_specific+''+event.name)]
            )
            ifpage_specificinmenu_entries:
                self.assertTrue(bool(view))
            #TDEFIXME:pagedeletionnotdonein13.3forIntroduction/Location,difficulttofix
            #withoutwebsite.event.menumodel(orcrappycodebasedonname)
            #else:
            #    self.assertFalse(bool(view))


classTestEventOnlineCommon(TestEventCommon,EventDtPatcher):
    ...
    defsetUpClass(cls):
        ...
        cls.event_customer.write({
            'website_description':'<p>Iamyourbestcustomer,%s</p>'%cls.event_customer.name,
        })
        cls.event_customer2.write({
            'website_description':'<p>Iamyourbestcustomer,%s</p>'%cls.event_customer2.name,
        })
