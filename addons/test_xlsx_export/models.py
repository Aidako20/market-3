...
classNewModel(models.Model):
    ...
    defname_get(self):
        return[(record.id,"%s:%s"%(self._name,record.value))forrecordinself]

classGroupOperator(models.Model):
    _name='export.group_operator'
    _description='ExportGroupOperator'
...
classGroupOperatorO2M(models.Model):
    _name='export.group_operator.one2many'
    _description='ExportGroupOperatorOne2Many'

    parent_id=fields.Many2one('export.group_operator')
    value=fields.Integer()

classComputedBinary(models.Model):
    _name='export.computed.binary'
    _description='Exportcomputedbinary'

    binary_field=fields.Binary(compute='_compute_binary_field')

    def_compute_binary_field(self):
        #Thiskindofcomputedbinaryfieldisobviouslyabadidea,
        #butsincetheORMsupportsit,theexportalsoneedstohandleit.
        self.binary_field=["computedvalue"]
