...
classTestMultiCompanySetup(TestMailCommon,TestRecipients):
    ...
    @users('employee')
    deftest_notify_reply_to_computation(self):
    ...
    @users('employee_c2')
    deftest_notify_reply_to_computation_mc(self):
        """Testreply-tocomputationinmulticompanymode.Addnotablytests
        dependingonuserandrecordscompany_id/company_ids."""
        company_3=self.env['res.company'].sudo().create({'name':'ELIT'})

        #Test1:nocompany_idfield
        test_record=self.env['mail.test.gateway'].browse(self.test_record.ids)
        res=test_record._notify_get_reply_to()
        self.assertEqual(
            res[test_record.id],
        ...
        #Test2:MCenvironmentgetdefaultvaluefromenv
        ...
        #Test3:getcompanyfromrecord(company_idfield)
        test_records=self.env['mail.test.multi.company'].create([
            {'name':'Test1',
    ...
    deftest_systray_get_activities(self):
        ...
        test_activity=next(
            aforainuser_admin.systray_get_activities()
            ifa['model']=='mail.test.multi.company.with.activity'
        )
        self.assertEqual(
            test_activity,
            {
                ...
                "name":"TestMultiCompanyMailWithActivity",
                "overdue_count":0,
                "planned_count":0,
                "today_count":2,
                "total_count":2,
        ...
        test_activity=next(
            aforainuser_admin.with_context(allowed_company_ids=[self.company_2.id]).systray_get_activities()
            ifa['model']=='mail.test.multi.company.with.activity'
        )
        self.assertEqual(
            test_activity,
