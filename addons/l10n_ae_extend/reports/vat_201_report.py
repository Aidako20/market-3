...
VAT_SALE_TYPE = ['local_sale', 'inside_gcc_sale', 'outside_gcc_sale', 'designated_zone_sale']
VAT_PURCHASE_TYPE = ['local_purchase', 'inside_gcc_purchase', 'outside_gcc_purchase', 'designated_zone_purchase']
...
class ReportVat201(models.AbstractModel):
    ...
    def get_subtotal(self, line, tax_ids):
        return line.price_subtotal if any([tax_id for tax_id in tax_ids if tax_id.tax_type in ['vat', 'customs'] and tax_id.amount != 0.0]) else 0.0

    def get_tax_amount(self, tax_ids):
        return sum([tax_id.amount for tax_id in tax_ids if tax_id.tax_id.tax_type == 'vat'])

    def get_invoice_data_for_local(self, data, vat_type):
        invoices = self.get_invoice_ids(data, ['out_invoice', 'out_refund'], vat_type, False)
        data_dict = {'amount': 0.0, 'total_exempted_amount': 0.0, 'total_exempted_adjustment': 0.0, 'adjustment': 0.0, 'tax_amount':0.0, 'return_tax_amount': 0.0, 'total_zero_amount': 0.0, 'zero_adjustment': 0.0}
        for invoice_id in invoices:
            tax_amount = self.get_tax_amount(invoice_id.tax_line_ids)
            check_line_vat_tax = 0.0
            for line in invoice_id.invoice_line_ids:
                data_dict.update(self.set_zero_exempted_amount(data_dict, invoice_id, line, line.invoice_line_tax_ids))
                check_line_vat_tax += self.get_subtotal(line, line.invoice_line_tax_ids)
    ...
    def set_zero_exempted_amount(self, data_dict, invoice_id, line, tax_line_ids):
        for tax_id in tax_line_ids:
            if tax_id.amount == 0.0:
                if invoice_id.type in ['out_invoice',  'in_invoice']:
                    data_dict['total_exempted_amount'] += line.price_subtotal if tax_id.tax_type == 'exempted' else 0.0
                    data_dict['total_zero_amount'] += line.price_subtotal if tax_id.tax_type != 'exempted' else 0.0
                elif invoice_id.type in ['out_refund', 'in_refund']:
                    data_dict['total_exempted_adjustment'] += line.price_subtotal if tax_id.tax_type == 'exempted' else 0.0
                    data_dict['zero_adjustment'] += line.price_subtotal if tax_id.tax_type != 'exempted' else 0.0
        return data_dict

    def get_invoice_data_for_local_purchase(self, data, vat_type):
        invoices = self.get_invoice_ids(data, ['in_invoice', 'in_refund'], vat_type, False)
    ...
    def get_data(self, invoices):
        ...
        for invoice_id in invoices:
            ...
            for line in invoice_id.invoice_line_ids:
                ...
                data_dict.update(self.set_zero_exempted_amount(data_dict,invoice_id, line, tax_line_ids))
    ...
    def get_zero_vat_purchase(self, data):
        return self.get_invoice_data_for_local_purchase(data, VAT_PURCHASE_TYPE)

    def get_total_purchase(self, data):
        reverse_dict = self.get_reverse_charge_data(data)
        all_type_dict = self.get_invoice_data_for_local_purchase(data, VAT_PURCHASE_TYPE)
        all_type_dict['amount'] += reverse_dict['amount'] + all_type_dict['total_zero_amount'] + all_type_dict['total_exempted_amount']
        all_type_dict['tax_amount'] += reverse_dict['tax_amount']
        all_type_dict['return_tax_amount'] += reverse_dict['return_tax_amount']
        all_type_dict['adjustment'] += reverse_dict['adjustment'] + all_type_dict['zero_adjustment'] + all_type_dict['total_exempted_adjustment']
        return all_type_dict

    def get_reverse_charge_data(self, data):
        invoices = self.get_invoice_ids(data, ['in_invoice', 'in_refund'], VAT_PURCHASE_TYPE, True)
        return self.get_data(invoices)
