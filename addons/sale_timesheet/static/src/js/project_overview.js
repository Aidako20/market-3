flectra.define('sale_timesheet.project_overview',function(require){
    ...
    constController=qweb.Controller.extend({
        ...
        _onFoldToggle(ev){
            ...
            constshouldOpen=ev.target.classList.contains('fa-caret-right');
            if(model==='sale.order'&&shouldOpen){
                this._openSaleOrder(resId);
            }
            elseif(model==='sale.order.line'&&shouldOpen){
                this._openSaleOrderLine(resId);
            }
            else{
                consttargetClass=`.${model.replace(/\./g,'_')}_${resId||'None'}`;
        ...
        _openSaleOrder(id){
            id=id||'None';
            consttargetClass=`.sale_order_${id}`;
        ...
        _openSaleOrderLine(id){
            id=id||'None';
            consttargetClass=`.sale_order_line_${id}`;
            this.$(`.o_timesheet_forecast_hr_employee${targetClass}`).show();
        },
    });

    varProjectOverview=qweb.View.extend({
        withSearchBar:true,
