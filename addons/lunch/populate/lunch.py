...
classLunchProductCategory(models.Model):
    ...
    def_populate_factories(self):
        #TODOtopping_ids_{1,2,3},toppping_label_{1,2,3},topping_quantity{1,2,3}
        ...
        return[
            ...
            ('company_id',populate.iterate(
                ...
                [1,1]+[2/(len(company_ids)or1)]*len(company_ids))),
        ]
...
classLunchProduct(models.Model):
    ...
    def_populate_factories(self):
        ...
        defget_price(random=None,**kwargs):
            returnrandom.randint(1,500)/10
        ...
        category_by_company={k:list(v)fork,vingroupby(category_records,key=lambdarec:rec['company_id'].id)}

        supplier_ids=self.env.registry.populated_models['lunch.supplier']
        company_by_supplier={rec.id:rec.company_id.idforrecinself.env['lunch.supplier'].browse(supplier_ids)}

        defget_category(random=None,values=None,**kwargs):
            company_id=company_by_supplier[values['supplier_id']]
            returnrandom.choice(category_by_company[company_id]).id
...
classLunchSupplier(models.Model):
    ...
    def_populate_factories(self):
        ...
        defget_email_time(random=None,**kwargs):
            returnrandom.randint(0,120)/10

        defget_location_ids(random=None,**kwargs):
...
classLunchOrder(models.Model):
    ...
    def_populate_factories(self):
        #TODOtopping_ids_{1,2,3},topping_label_{1,3},topping_quantity_{1,3}
        user_ids=self.env.registry.populated_models['res.users']
...
classLunchAlert(models.Model):
    ...
    def_populate_factories(self):
        ...
        defget_notification_time(random=None,**kwargs):
            returnrandom.randint(0,120)/10

        defget_until_date(random=None,**kwargs):
            delta=random.randint(-731,731)
            returndatetime(2020,1,1)+timedelta(days=delta)

        defget_location_ids(random=None,**kwargs):
            nb_max=len(location_ids)
            start=random.randint(0,nb_max)
            end=random.randint(start,nb_max)
            returnlocation_ids[start:end]
        ...
        return[
            ...
            ('message',populate.constant('<strong>alertmessage{counter}</strong>')),
